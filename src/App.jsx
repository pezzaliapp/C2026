...
// ─── CATALOGO BRACCI (PDF "Misure Generali Bracci / Tipo Veicoli") ───────────
//
// Nuovo percorso "Seleziona il ponte in funzione del veicolo": l'utente clicca
// direttamente su uno dei bracci del PDF ufficiale Cascos, poi sceglie la
// pavimentazione (industriale = modelli S senza pedana / non industriale = con
// pedana) e viene ricollegato in toto ai prodotti della logica attuale.

const BRACCI_CATALOGO = [
  {
    id: 'c32e',
    label: 'C3.2 E',
    portata: '3,2 ton',
...
// Seleziona i prodotti (2 colonne) associati al braccio scelto e alla
// pavimentazione. Riutilizza in toto il catalogo della logica attuale.
function selectProductsByBraccio(products, braccioId, pavimentazione) {
  const braccio = BRACCI_CATALOGO.find(b => b.id === braccioId);
  if (!braccio) return [];
  const ids = braccio.prodotti[pavimentazione] || [];
  return products
...
function useProducts() {
  ...
  // tipoPrezzo: 'netto' (default, comportamento originale) oppure 'lordo'.
  // - netto  → cerca la colonna contenente "netto"  (es. "Netto Riv. (€)")
  // - lordo  → cerca la colonna contenente "lordo" o, in alternativa, "listino"
  const importFromExcel = useCallback((file, tipoPrezzo = 'netto') => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          let colPre;
          if (tipoPrezzo === 'lordo') {
            colPre = headers.findIndex(h => typeof h === 'string' && h.toLowerCase().includes('lordo'));
            if (colPre === -1) {
              colPre = headers.findIndex(h => typeof h === 'string' && h.toLowerCase().includes('listino'));
            }
            if (colPre === -1) {
              reject(new Error('Colonna prezzo lordo non trovata (attesa colonna "Lordo" o "Listino")'));
              return;
            }
          } else {
            colPre = headers.findIndex(h => typeof h === 'string' && h.toLowerCase().includes('netto'));
            if (colPre === -1) {
              reject(new Error('Colonna prezzo netto non trovata (attesa colonna "Netto")'));
              return;
            }
          }
          ...
          // Salva prezzi e metadata in localStorage (incluso il tipo di prezzo)
          const info = {
            fileName: file.name,
            tipoPrezzo,
...
Pavimentazione: Universale (4 colonne)`;
    } else if (isBracciFlow) {
      const floorLabel = FLOOR_TYPES.find(f => f.id === config.pavimentazione)?.label || '—';
      const braccio = BRACCI_CATALOGO.find(b => b.id === config.braccio);
      dettagliConfig = `Tipo sollevatore: 2 Colonne
Selezione: In funzione del veicolo (braccio ${braccio?.label || '—'})
Pavimentazione: ${floorLabel}
Range bracci: ${braccio ? `Min ${braccio.minMm} mm - Max ${braccio.maxMm} mm` : '—'}
...
Note tecniche: ${product.noteTecniche || '—'}`;
  ...
  // Riga costi opzionali (solo listino lordo)
  let righeCosti = '';
  if (isLordo && costiExtra) {
    if (costiExtra.installazione > 0) righeCosti += `Costi installazione: ${formatPrice(costiExtra.installazione)}\n`;
    if (costiExtra.trasferta > 0)     righeCosti += `Trasferta: ${formatPrice(costiExtra.trasferta)}\n`;
    if (costiExtra.trasporto > 0)     righeCosti += `Trasporto: ${formatPrice(costiExtra.trasporto)}\n`;
  }
...
function Logo() {
  return (
    <div className="flex items-center gap-3">
      ...
      <div>
        <div className="text-sm font-bold text-white leading-none">QuoteFlow C2026</div>
...
function ProductCard({ product, isRecommended, onSelect, mode, alreadyInCart, tipoPrezzo = 'netto' }) {
        ...
        <div className="text-right">
          {hasPrice ? (
            <>
              ...
              <div className="text-xs text-slate-500">{tipoPrezzo === 'lordo' ? 'prezzo lordo' : 'prezzo netto'}</div>
            </>
          ) : (
            <>
...
function DashboardView({ onStart, onStartBracci, onImport, importStatus, listinoInfo, onResetPrices }) {
  ...
  const handleFileDrop = (e) => {
    ...
    if (file) onImport(file, tipoPrezzoImport);
  };
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      {listinoInfo ? (
        <div className="space-y-4">
          ...
          {/* NUOVO — Percorso guidato dai bracci del catalogo Cascos */}
          <button
            onClick={() => onStartBracci('quote')}
            className="w-full glass glass-hover rounded-xl p-5 text-left hover:scale-[1.01] transition-all border border-violet-500/30 bg-violet-500/5"
          >
            <div className="flex items-center gap-4">
              <Ruler size={24} className="text-violet-400 flex-shrink-0" />
              <div>
                <div className="font-bold text-white mb-1">Seleziona il ponte in funzione del veicolo</div>
      ...
      <div className="glass rounded-xl p-4">
        ...
        {listinoInfo ? (
          <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3 py-2 mb-3">
            <div className="min-w-0 flex-1">
              <p className="text-xs text-emerald-300 font-semibold">
                Listino attivo · Prezzo {listinoInfo.tipoPrezzo === 'lordo' ? 'Lordo' : 'Netto'}
              </p>
              ...
              <p className="text-xs mt-0.5">
                {listinoInfo.tipoPrezzo === 'lordo'
                  ? <span className="text-amber-300">Sconti e costi opzionali (installazione, trasferta, trasporto) abilitati</span>
                  : <span className="text-sky-300">Prezzo netto — gestione sconti non consentita</span>}
              </p>
            </div>
            <button
          ...
          <p className="text-xs text-slate-500 mb-3">
            Nessun listino caricato. Importa l'Excel scegliendo se caricare il prezzo netto o il prezzo lordo.
            Colonne attese: Riferimento + Netto Riv. (€) oppure Lordo/Listino (€).
          </p>
        )}

        {/* NUOVO — Scelta tipo prezzo da importare */}
        <div className="mb-3">
          <div className="text-xs text-slate-400 mb-2">Tipo di prezzo da importare</div>
          <div className="flex gap-2">
            <button
              onClick={() => setTipoPrezzoImport('netto')}
              className={`flex-1 rounded-lg py-2.5 px-3 text-left transition-colors border ${
                tipoPrezzoImport === 'netto'
                  ? 'bg-blue-600/30 border-blue-500/60 text-white'
                  : 'glass border-slate-700 text-slate-400 hover:text-white'
              }`}
            >
              <div className="text-sm font-semibold">Prezzo Netto</div>
              <div className="text-xs opacity-80">Nessuno sconto applicabile</div>
            </button>
            <button
              onClick={() => setTipoPrezzoImport('lordo')}
              className={`flex-1 rounded-lg py-2.5 px-3 text-left transition-colors border ${
                tipoPrezzoImport === 'lordo'
                  ? 'bg-amber-600/30 border-amber-500/60 text-white'
                  : 'glass border-slate-700 text-slate-400 hover:text-white'
              }`}
            >
              <div className="text-sm font-semibold">Prezzo Lordo</div>
              <div className="text-xs opacity-80">Sconti + installazione, trasferta, trasporto</div>
            </button>
          </div>
        </div>
...
function ConfiguratorView({ mode, products, onResult, onBack, initialFlow = null }) {
  ...
  // ─── Handlers percorso bracci ─────────────────────────────────────────────
  // L'utente clicca su uno dei bracci del catalogo (PDF misure Cascos),
  // poi sceglie industriale / non industriale: da lì si ricollega in toto
  // ai prodotti e alle viste della logica originale.
  const handleBraccio = (id) => { setBraccio(id); setStep(1); };
  const handleBraccioFloor = (pav) => {
    setPav(pav);
    const results = selectProductsByBraccio(products, braccio, pav);
    onResult({
  ...
  const handleBackInternal = () => {
    ...
    if (step === 0) {
      ...
      setPav(null); setVeicolo(null); setDistanza(null); setImpiego(null); setBraccio(null);
      return;
    }
    setStep(s => s - 1);
  };
  ...
  return (
    <div className="animate-fade-in space-y-6">
      ...
      {tipoSollevatore === null && (
        <div className="animate-slide-up">
          ...
          <div className="space-y-3">
            ...
            {/* NUOVO — percorso guidato dai bracci */}
            <button
              onClick={() => { setTipoSollevatore('bracci'); setStep(0); }}
              className="w-full glass glass-hover rounded-xl p-5 text-left flex items-center gap-4 transition-all hover:scale-[1.01] border border-violet-500/30 bg-violet-500/5"
            >
      ...
      {/* ─── FLOW BRACCI (Selezione in funzione del veicolo) ─────────────── */}
      {tipoSollevatore === 'bracci' && step === 0 && (
        <div className="animate-slide-up">
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-xl font-bold text-white">Scegli il Braccio</h2>
            <Badge text="Catalogo misure Cascos" color="violet" />
          </div>
          <p className="text-sm text-slate-400 mb-5">
            Seleziona il braccio adatto al veicolo da sollevare: le misure Min/Max sono
            la distanza dei punti di presa (dal catalogo ufficiale Cascos).
          </p>
          <div className="space-y-3">
            {BRACCI_CATALOGO.map(b => (
              <button
                key={b.id}
      ...
      {tipoSollevatore === 'bracci' && step === 1 && (
        <div className="animate-slide-up">
          <div className="flex items-center gap-2 mb-1">
            <h2 className="text-xl font-bold text-white">Tipo di Pavimentazione</h2>
            <Badge
              text={BRACCI_CATALOGO.find(b => b.id === braccio)?.label || ''}
              color="violet"
            />
          </div>
          <p className="text-sm text-slate-400 mb-5">
            Il ponte è per pavimento industriale o non industriale? La scelta determina
            la versione (senza pedana / con pedana) del modello corrispondente.
          </p>
          <div className="space-y-3">
            {FLOOR_TYPES.map(f => (
              <button
                key={f.id}
                onClick={() => handleBraccioFloor(f.id)}
...
function ResultsView({ mode, config, cartItems, onAddToCart, onGoToQuote, onBack, onReset, tipoPrezzo = 'netto' }) {
...
function QuoteView({ mode, items, onUpdateQty, onRemoveItem, onAddMore, onBack, onReset, tipoPrezzo = 'netto' }) {
  ...
  // NUOVO — costi opzionali disponibili SOLO con listino a prezzo lordo
  const [costoInstallazione, setCostoInstallazione] = useState(0);
  const [costoTrasferta, setCostoTrasferta]         = useState(0);
          ...
          <div className="p-4 border-b border-slate-700 print:border-gray-300">
            <div className="flex flex-col items-end gap-1 text-sm">
              ...
              {isLordo && costiExtra.installazione > 0 && (
                <div className="flex gap-6 text-slate-400 print:text-gray-500">
                  <span>Costi installazione</span>
                  <span className="font-mono">{formatPrice(costiExtra.installazione)}</span>
                </div>
              )}
              {isLordo && costiExtra.trasferta > 0 && (
                <div className="flex gap-6 text-slate-400 print:text-gray-500">
                  <span>Trasferta</span>
                  <span className="font-mono">{formatPrice(costiExtra.trasferta)}</span>
                </div>
              )}
              {isLordo && costiExtra.trasporto > 0 && (
                <div className="flex gap-6 text-slate-400 print:text-gray-500">
                  <span>Trasporto</span>
                  <span className="font-mono">{formatPrice(costiExtra.trasporto)}</span>
                </div>
              )}
              <div className="flex gap-6 text-white print:text-black text-lg font-bold border-t border-slate-600 print:border-gray-300 pt-2 mt-1">
                <span>{isLordo ? 'Totale' : 'Totale Netto'}</span>
    ...
    <div className="animate-fade-in space-y-5">
      ...
      <div className="glass rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between mb-1">
          <div className="text-sm font-semibold text-slate-300">Dettagli {docType}</div>
          <Badge
            text={isLordo ? 'Listino Lordo' : 'Listino Netto'}
            color={isLordo ? 'amber' : 'blue'}
          />
        </div>
        ...
        {isLordo ? (
          <>
            ...
            <div className="pt-1">
              <div className="text-xs text-slate-400 mb-2">Costi opzionali (€, IVA esclusa)</div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">Installazione</label>
                  <input
                    className={inputCls}
                    type="number" min="0" placeholder="0"
                    value={costoInstallazione || ''}
                    onChange={e => setCostoInstallazione(Math.max(0, parseFloat(e.target.value) || 0))}
                  />
                </div>
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">Trasferta</label>
                  <input
                    className={inputCls}
                    type="number" min="0" placeholder="0"
                    value={costoTrasferta || ''}
                    onChange={e => setCostoTrasferta(Math.max(0, parseFloat(e.target.value) || 0))}
                  />
                </div>
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">Trasporto</label>
                  <input
                    className={inputCls}
                    type="number" min="0" placeholder="0"
                    value={costoTrasporto || ''}
                    onChange={e => setCostoTrasporto(Math.max(0, parseFloat(e.target.value) || 0))}
                  />
                </div>
              </div>
            </div>
          ...
          <div className="flex items-start gap-2 bg-sky-500/10 border border-sky-500/20 rounded-lg px-3 py-2">
            ...
            <p className="text-xs text-slate-400">
              Listino caricato a <strong className="text-sky-300">prezzo netto</strong>:
              la gestione degli sconti e dei costi opzionali non è consentita.
              Per abilitarli, importa il listino con prezzo lordo dalla dashboard.
            </p>
          </div>
        )}
      ...
      <div className="glass rounded-xl p-4">
        ...
        {isLordo && costiExtra.installazione > 0 && (
          <div className="flex justify-between items-center text-sm text-slate-400 mb-1">
            <span>Costi installazione</span>
            <span>{formatPrice(costiExtra.installazione)}</span>
          </div>
        )}
        {isLordo && costiExtra.trasferta > 0 && (
          <div className="flex justify-between items-center text-sm text-slate-400 mb-1">
            <span>Trasferta</span>
            <span>{formatPrice(costiExtra.trasferta)}</span>
          </div>
        )}
        {isLordo && costiExtra.trasporto > 0 && (
          <div className="flex justify-between items-center text-sm text-slate-400 mb-1">
            <span>Trasporto</span>
            <span>{formatPrice(costiExtra.trasporto)}</span>
          </div>
        )}
...
export default function App() {
  ...
  // NUOVO — flusso iniziale del configuratore: null (standard) o 'bracci'
  const [configFlow, setConfigFlow]       = useState(null);

  // Tipo di prezzo del listino attivo ('netto' default = comportamento originale)
  const tipoPrezzo = listinoInfo?.tipoPrezzo || 'netto';
  ...
  // NUOVO — avvia il percorso "Seleziona il ponte in funzione del veicolo"
  const handleStartBracci    = (m) => {
    setMode(m);
    setCartItems([]);
    setConfig(null);
  ...
  const handleAddToCart = (product) => {
    setCartItems(items => {
      const existingIdx = items.findIndex(it => {
        ...
        // Le righe del flusso bracci matchano solo tra loro (stesso braccio + pavimentazione)
        if ((it.config.flow === 'bracci') !== (config.flow === 'bracci')) return false;
        if (config.flow === 'bracci') {
          return it.config.braccio === config.braccio &&
                 it.config.pavimentazione === config.pavimentazione;
        }
  ...
  const handleImport = async (file, tipoPrezzoImport = 'netto') => {
    try {
      const count = await importFromExcel(file, tipoPrezzoImport);
      setImportStatus(`${count} prodotti aggiornati (${tipoPrezzoImport})`);
    } catch (err) {
      setImportStatus(err?.message ? `Errore import — ${err.message}` : 'Errore import — verifica formato');
    }
  };
